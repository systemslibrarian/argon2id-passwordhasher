...
| --- | --- | --- |
| Build L1 | Provenance exists, describing how the artifact was built | **Met** — `actions/attest-build-provenance` attaches signed provenance to every released `.nupkg` |
| Build L2 | Provenance is generated by a hosted build platform and signed; downstream can verify | **Met** — builds run on GitHub-hosted runners; verify with `gh attestation verify --repo systemslibrarian/argon2id-passwordhasher` |
| Build L3 | Build runs on a hardened platform; provenance is unforgeable even by the project's own maintainers (isolated/reusable workflow) | **Not met** — would require moving the release build into an isolated reusable workflow (e.g. `slsa-github-generator`); on the roadmap |

Supporting properties, independent of the level claim:

- **Deterministic builds, verified in CI.** Every CI run builds and packs
  the libraries twice from a clean tree and fails if the assemblies are
  not byte-for-byte identical (`repro-build` job in `ci.yml`). Anyone can
  rebuild the tagged commit and compare hashes against the published
  package.
- **Pinned SDK** (`global.json`) and locked, minimal runtime dependency
  set.
- **No maintainer-held signing keys in the build path** — provenance
  signing uses GitHub's OIDC-backed Sigstore flow, so there is no
...
- Deterministic builds + SourceLink + build-provenance attestations,
  with reproducibility verified on every CI run (double-build + compare).
- Nightly coverage-guided fuzzing of the hash-format parser (SharpFuzz /
  libFuzzer), with the seed corpus replayed as unit tests on every CI run.
- Nightly differential testing against a second, independent Argon2
  implementation (Isopoh) with randomized parameters.
- Weekly mutation testing (Stryker.NET) with a minimum-score gate.
- Line-coverage threshold gate on every CI run.
